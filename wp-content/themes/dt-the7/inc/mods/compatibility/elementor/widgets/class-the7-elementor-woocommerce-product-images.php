...
class The7_Elementor_Woocommerce_Product_Images extends The7_Elementor_Widget_Base {
	...
	public function get_icon() {
		return 'eicon-product-images the7-widget';
	}
	...
	public function get_script_depends() {
		wp_register_script( 'jquery-mousewheel', PRESSCORE_THEME_URI . '/js-lite/compatibility/woocommerce/jquery-mousewheel.min.js', [ 'jquery' ], THE7_VERSION, true );
		if ( Plugin::$instance->preview->is_preview_mode() ) {
			wp_register_script( 'the7-woocommerce-product-images-widget-preview', PRESSCORE_ADMIN_URI . '/assets/js/elementor/wc-widget-preview.js', [ 'jquery' ], THE7_VERSION, true );

			return [ 'the7-woocommerce-product-images-widget-preview', 'jquery-mousewheel', 'zoom' ];
		}

		return [ 'flexslider', 'jquery-mousewheel', 'zoom' ];
	}

	public function get_keywords() {
		return [ 'woocommerce', 'shop', 'store', 'image', 'product', 'the7', 'gallery', 'lightbox' ];
	}
	...
	public function render() {
		...
		if ( ! empty( $combined_attachment_ids ) && 'disabled' !== $settings['thumbs_direction'] ) {
			ob_start();
			$item_exist = false;
			?>
	...
	protected function add_big_image_style_controls() {
		...
		$this->add_group_control( Group_Control_Border::get_type(), [
			...
			'selector'  => '{{WRAPPER}} .dt-product-gallery .flexslider .flex-viewport',
			'separator' => 'before',
		] );
		...
		$this->add_responsive_control( 'gallery_image_border_radius', [
			...
			'size_units' => [ 'px', '%' ],
			'selectors'  => [
				'#main {{WRAPPER}} .dt-product-gallery .flexslider .flex-viewport' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}} !important',
			],
		] );
		...
		$this->add_control( 'gallery_spacing', [
			...
			'size_units' => [ 'px' ],
			'selectors'  => [
				'{{WRAPPER}} .thumb-position-right .dt-product-gallery'  => 'margin-right: {{SIZE}}{{UNIT}}',
				'{{WRAPPER}} .thumb-position-left .dt-product-gallery'   => 'margin-left: {{SIZE}}{{UNIT}}',
				'{{WRAPPER}} .thumb-position-bottom .dt-product-gallery' => 'margin-bottom: {{SIZE}}{{UNIT}}',
